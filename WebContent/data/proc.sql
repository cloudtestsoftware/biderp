...
--------------------------------------------------------
 -- Unable to render PROCEDURE DDL for object ERP.INSERT_ADDUSER with DBMS_METADATA attempting internal generator.
...
Begin
		select objid into companyId from table_company where rownum=1;
...
-- Unable to render PROCEDURE DDL for object ERP.INSERT_BIDPROJECT with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     INSERT_BIDPROJECT (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.INSERT_BIDREQUEST with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE INSERT_BIDREQUEST (pnObjid raw) IS 
...
CURSOR m_Invitation_cur IS Select
         ...
         s.phone,
         b.rating,
         'Not Invited' as status,
         r.genuser
         from table_supplier s, table_bizprofile b, table_bidrequest r 
where r.objid=pnObjid
    and  r.bidrequest2bizdomain=b.bizprofile2bizdomain 
    and b.rating>=r.rating 
    and b.bizprofile2supplier=s.objid
	 and not exists (select *from table_Invitation where Invitation2Bidrequest=r.objid and 
     email=s.email);
...
--Insert records in Invitation
	...
	INSERT INTO table_Invitation(
        ...
        pnObjid,
		pnObjid,
		i_Invitation_cur.objid,
		i_Invitation_cur.genuser,
		sysdate,
		null,
		null
...
-- Unable to render PROCEDURE DDL for object ERP.INSERT_BIDWON with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     INSERT_BIDWON (BidReqId raw,ProjectId raw,VendBidNo raw,guser varchar,grpuser varchar) IS
...
-- Unable to render PROCEDURE DDL for object ERP.INSERT_BUDGET with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     INSERT_BUDGET (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.INSERT_CALENDAR with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     INSERT_CALENDAR (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.INSERT_INVITATION with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE INSERT_INVITATION (pnObjid raw) IS 

--Constants for status
STATUS_OPEN	varchar2(20):='1';
STATUS_INVITED	varchar2(20):='INVITED';
...
CURSOR m_VendorBid_cur IS select
    ...
    g.bidduedate,
    r.currencycode,
    g.address || ', ' ||g.zipcode||','||g.state||',' || g.contactname || ',' ||g.phone as contact,
    i.name suppliername,
    i.email bidderlogin, 
    i.phone,
    i.invitation2bidrequest
from table_bidrequest r, table_general g, table_invitation i
where i.invitation2bidrequest=r.objid and
  r.objid=g.general2bidrequest(+)
	 and i.objid=pnObjid 
	 and not exists (select *from table_VendorBid where originid= i.objid);
...
--opening cursor m_VendorBid_cur
...
--Insert records in VendorBid
	...
	INSERT INTO table_VendorBid(
        ...
        vendbidid,
		i_VendorBid_cur.projectname,
		i_VendorBid_cur.bidderlogin,
		i_VendorBid_cur.CURRENCYCODE,
		i_VendorBid_cur.bidduedate,
		'1',
		i_VendorBid_cur.description,
        30,
        i_VendorBid_cur.phone,
        'your email',
        'your url',
        i_VendorBid_cur.contact,
        null,
		i_VendorBid_cur.originid,
		i_VendorBid_cur.Invitation2BidRequest,
		i_VendorBid_cur.bidderlogin,
        i_VendorBid_cur.bidderlogin,
		sysdate,
		null,
...
-- Unable to render PROCEDURE DDL for object ERP.INSERT_MILESTONE with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     INSERT_MILESTONE (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.INSERT_PARTPRICE with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     INSERT_PARTPRICE (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.INSERT_PARTS with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     INSERT_PARTS (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.INSERT_PROFILE with DBMS_METADATA attempting internal generator.
...
-- Unable to render PROCEDURE DDL for object ERP.INSERT_PROJECT with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     INSERT_PROJECT (pnObjid raw) IS
...
CURSOR m_Estimation_cur IS Select distinct
	 ...
	 where 	p.objid=pnObjid
		...
		and m.groupuser=decode(j.projectcode,'CSI','admin@biderp.com',p.groupuser)
	 and not exists (select *from table_Estimation e
...
CURSOR m_BOQ_cur IS Select distinct
	 ...
	 where p.objid=pnObjid
		...
		and s.groupuser=decode(m.projectcode,'CSI','admin@biderp.com',p.groupuser)
...
-- Unable to render PROCEDURE DDL for object ERP.INSERT_PROJECTLEAD with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     INSERT_PROJECTLEAD (bidReqId raw,VendorbidId raw,vuser varchar,vgroupuser varchar) IS
...
-- Unable to render PROCEDURE DDL for object ERP.INSERT_QUOTE with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     INSERT_QUOTE (pnObjid raw) IS
...
CURSOR m_JobMaster_cur IS Select
    ...
    'Quote' moduser
     ...
     and not exists (select * from table_jobmaster j,table_project p
		where p.objid=j.jobmaster2project 
        and p.name=q.name 
        --and j.destinitionid=q.quote2phase 
        and j.projectcode=m.projectcode and
...
CURSOR m_JobList_cur IS Select
    ...
    qm.objid=q.quotejobs2quotemaster and
     ...
     and not exists (select * from table_joblist j,table_project p
		where p.name=qt.name 
        and p.objid=j.joblist2project 
        --and j.destinitionid=qt.quote2phase 
        and j.originid=pnObjid 
        and q.quotejobs2quotemaster=qm.objid 
        and qm.quotemaster2quote=pnobjid);
  ...
  CURSOR m_Parts_cur IS Select
   ...
   where p.DESTINITIONID=pnObjid and p.genuser='Quote' and p.name=q.name
			and m.jobmaster2project=p.objid 
            and j.joblist2jobmaster=m.objid 
            and pt.parts2joblist=j.objid 
            and pt.genuser='Quote' );
   ...
   CURSOR m_TaskResource_cur IS Select
	 ...
	 where q.objid=pnObjid and  qm.quotemaster2quote=q.objid 
     and qj.quotejobs2quotemaster=qm.objid and qr.quoteResource2quotejobs=qj.objid
	 and not exists (select * from table_taskresource tr,table_joblist j, table_jobmaster m , table_Project p 
   where p.DESTINITIONID=pnObjid and p.genuser='Quote' and p.name=q.name
			and m.jobmaster2project=p.objid and j.joblist2jobmaster=m.objid 
            and tr.taskresource2joblist=j.objid and tr.genuser='Quote' );
...
--Insert records in QuoteMaster
 ...
 --Insert records in QuoteJobs
 	...
 	INSERT INTO table_QuoteJobs(
 		...
 		0,
 		pnObjid,  --storing QuoteJobs2QuoteMaster= quote id temporarily will be changed while updating relation
 		quotejobId,
 		pnObjid,
...
select count(*) into cntConsole from table_console where ORIGINID=pnObjid;

if cntConsole = 0 then
...
-- Unable to render PROCEDURE DDL for object ERP.INSERT_USER with DBMS_METADATA attempting internal generator.
...
-- Unable to render PROCEDURE DDL for object ERP.INSERT_VENDORBID with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE INSERT_VENDORBID (pnObjid raw) IS 
--Constants for status
STATUS_OPEN	varchar2(20):='1';
STATUS_INVITED	varchar2(20):='INVITED';
...
CURSOR m_ItemPrice_cur IS Select
		...
		p.QntRequest,
		p.UmCode,
        p.objid ItemPrice2PartBid,
		v.groupuser
		from table_PartBid p, table_invitation i, table_vendorbid v
	 where v.objid=pnObjid 
     and v.originid=i.objid
     and i.INVITATION2BIDREQUEST=p.PartBid2BIDREQUEST 
...
CURSOR m_QuizReply_cur IS Select
        ...
        q.POINT,
		i.genuser groupuser
		from table_bidquiz q,table_invitation i, table_vendorbid v
	 where v.objid=pnObjid 
           and v.originid=i.objid
           and i.INVITATION2BIDREQUEST=q.BIDQUIZ2BIDREQUEST
...
--Insert records in QuizReply
	...
	INSERT INTO table_QuizReply(
       objid,
        ...
        pnObjid,
		pnObjid,
		i_QuizReply_cur.destinitionId,
		i_QuizReply_cur.groupuser,
        i_QuizReply_cur.groupuser,
		sysdate,
		null,
		null
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_BIDPROJECT with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     UPDATE_BIDPROJECT (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_BIDREQUEST with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     UPDATE_BIDREQUEST (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_BIDWON with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     UPDATE_BIDWON (vendorBidNo raw,projectno raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_BOQPLAN with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     UPDATE_BOQPLAN (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_FINANCIALTERM with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     UPDATE_FINANCIALTERM (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_ITEMDISPATCH with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     UPDATE_ITEMDISPATCH (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_ITEMRECEIVED with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     UPDATE_ITEMRECEIVED (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_JOBLIST with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     UPDATE_JOBLIST (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_MAINCODE with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     UPDATE_MAINCODE (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_PARTS with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     UPDATE_PARTS (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_POINVOICE with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     UPDATE_POINVOICE (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_PROFILE with DBMS_METADATA attempting internal generator.
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_PROJECT with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     UPDATE_PROJECT (pnObjid raw) IS
...
--update parts2boq
  Begin
    ...
    where p.parts2joblist=i_BOQUP_cur.joblistid;
    --and p.parts2boq =pnObjid;
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_PROJECTCODE with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     UPDATE_PROJECTCODE (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_PURCHASEORDER with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     UPDATE_PURCHASEORDER (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_QUOTE with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     UPDATE_QUOTE (pnObjid raw) IS
...
--First get consoleid and messagequeue id
	...
	Begin
		...
		select objid into projectid from table_project where  originid=pnObjid and genuser='Quote' and name=quotename;
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_QUOTEJOBS with DBMS_METADATA attempting internal generator.
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_RESOURCEORDER with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     UPDATE_RESOURCEORDER (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_SUBCODE with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     UPDATE_SUBCODE (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_TASKCODE with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     UPDATE_TASKCODE (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_TASKRESOURCE with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     UPDATE_TASKRESOURCE (pnObjid raw) IS
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_VENDORBID with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     UPDATE_VENDORBID (pnObjid raw) IS
...
BEGIN
    ...
    Begin
		update table_QuizReply
			set QuizReply2Bids=pnObjid
			 where QuizReply2Vendorbid=pnObjid;
	...
	End;
     ...
     Begin
		update table_Attachment
			set Attachment2Bids=pnObjid
			 where Attachment2Vendorbid=pnObjid;
...
-- Unable to render PROCEDURE DDL for object ERP.UPDATE_WHATTODO with DBMS_METADATA attempting internal generator.
CREATE OR REPLACE PROCEDURE     UPDATE_WHATTODO (pnObjid raw) is
