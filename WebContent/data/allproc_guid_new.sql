...
Update table_joblist j set j.joblist2jobmaster=
(select m.objid from table_jobmaster m where m.jobmaster2project=projectid and
m.projectcode=j.projectcode and m.maincode=j.maincode and j.joblist2project=projectid) 
where  j.joblist2jobmaster=pnObjid;

EXCEPTION
		WHEN NO_DATA_FOUND THEN
		null;
end;
 --update parts and task resource with joblist id
 begin

Update table_parts p set p.parts2joblist=
(select j.objid from table_joblist j, table_jobmaster m where j.joblist2jobmaster=m.objid 
and  m.jobmaster2project=projectid and
m.projectcode=j.projectcode and m.maincode=j.maincode 
and p.subcode=j.subcode and j.joblist2project=projectid) 
where   p.parts2joblist=pnObjid;
