CREATE OR REPLACE PROCEDURE Insert_Budgetfundreleased(pnObjid NUMBER) IS 
...
CURSOR m_Transaction_cur IS Select
	 ...
	 where objid=pnObjid 
	 and not exists (select *from table_Transaction where Transaction2Budgetfundreleased=pnObjid);
