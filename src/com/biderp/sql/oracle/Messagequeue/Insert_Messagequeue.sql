CREATE OR REPLACE PROCEDURE Insert_Messagequeue(pnObjid NUMBER) IS 
...
CURSOR m_Participants_cur IS Select
	 ...
	 where objid=pnObjid 
	 and not exists (select *from table_Participants where Participants2Messagequeue=pnObjid);
