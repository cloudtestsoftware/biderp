	...
	public class TrtasksDao extends TrtasksImpl {
		Map<String, Cookie> cookies; 
		Map<String,String> userdata;
		protected String []deletetabs={"trtasks,"};
		protected String []childtabs={"trtasks,"};
		protected String []childtabnames={"Trtasks Facts,"};
		
		protected String [] maintype={"table"};
		protected String [] maincol={"objid","trtasks2rfq","name","stage","partno","note"};
	...
	protected String [] propTrtaskslist={"stage"};
		protected String [] codeTrtaskslist={};
		protected String [] relationTrtaskslist={"rfq:trtasks2rfq:list:"};

		public TrtasksDao(UriInfo uriInfo, HttpHeaders header) throws AuthenticationException{
			this.setObject("Trtasks");
		...
		public void setPostXml(String xml) throws DaoException{
			...
			for(String entity:entitys){
				String tmp="";
				if(entity.toLowerCase().contains("<trtasks>")){
					tmp=entity.replace("<?", "");
		...
		public Rows getTrtasksSummaryRows(){
			TemplateTable tab=this.doSelect(summarycol,summarysqldatatype,this.TrtasksFilter);
			ArrayList<String> chartcols=tu.getChartSelectColumns("Trtasks");
			...
			for(String chartcol:chartcols){
				ArrayList<String> datas= tu.getChartPropertyJSON("Trtasks", tab, chartcol);
		...
		public Rows getTrtasksRows(){
			...
			Rows rows=tu.getXMLRows(tab, "Trtasks",codeTrtaskslist,propTrtaskslist,relationTrtaskslist,maincolcaption,maindatadomain,this.getGroupuser(),bqn);
			ArrayList<Userdata> userdata=rows.getUserdata();
		...
		public Rows getTrtasksRowModified(){
			Rows rows=tu.getXMLRows(maindata, "Trtasks",codeTrtaskslist,propTrtaskslist,relationTrtaskslist,maincolcaption,maindatadomain,this.getGroupuser(),bqn);
			return rows;
		}
		...
		public void postTrtasksContainer() throws DaoException{
		...
		public Rows getTrtasksByFilter(){
			String newfilter=" groupuser='"+this.getGroupuser()+"'";
			if(!tu.isEmptyValue(this.getFilters())){
				newfilter+=" and trtasks2"+this.getFilters();
			}
			String sql= "select * from table_Trtasks where "+ newfilter;
			TemplateTable tab=tu.getResultSet(sql);
			Rows rows=tu.getXMLFilterRows(tab, "Trtasks",codeTrtaskslist,propTrtaskslist,relationTrtaskslist,maincol,maincolcaption,maindatadomain,this.getGroupuser(),bqn);
			ArrayList<Userdata> userdata=rows.getUserdata();
			Userdata data1= new Userdata("tabs",Arrays.asList(childtabs));
			Userdata data2= new Userdata("tabnames",Arrays.asList(childtabnames));
			Userdata data3= new Userdata("filters",Arrays.asList(maincolsearch));
