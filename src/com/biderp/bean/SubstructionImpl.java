	 ...
	 * @author S.K Jana Version 1.0 
	 ...
	 public class SubstructionImpl implements QueryEvent {
		 protected static Log logger = LogFactory.getLog(SubstructionImpl.class); 
		 ...
		 public String getParentobjid(){
		  ...
		  public  TemplateTable  doSelect(String[] column,String[] datatype,String parentfilter){
			...
			//do some custom pre query operation if any 
			 EventListener.registerPreQueryParent("Substruction",column,datatype);
			 ...
			 maindata=query.getTableResultset();
					...
					EventListener.registerPostQueryParent("Substruction",column,datatype);
			 return(maindata);
		 }
		 ...
		 public  TemplateTable  doSelectChild(String childname,String relfield,String pid,String[]column,String[]datatype,String childfilter){
			...
			EventListener.registerPreQueryChild("Substruction",childname,pid,relfield,column,datatype);
			...
			//Do some post query operation for child
					EventListener.registerPostQueryChild("Substruction",childname,pid,relfield,column,datatype);
			}
			return(data);
		}
		public  boolean   doDelete(String[] childtabs){
			...
			String sql=query.removeSql("Substruction",this.getParentobjid(),childtabs,groupuser);
			if(!tu.isEmptyValue(sql)){
				tu.applyObjectRuleForDelete("Substruction",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_BEFORE,this.getParentobjid());
				EventListener.registerPreDeleteEvent("Substruction",this.getParentobjid());
				query.setQuery(sql);
				if(ACONST.GENERATE_LOG)
					logger.info(query.getQuery());
				if (query.getTableResultset().getRowCount()>0){
					// Do some post delete operation
					tu.applyObjectRuleForDelete("Substruction",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_AFTER,this.getParentobjid());
					EventListener.registerPostDeleteEvent("Substruction",this.getParentobjid());
					return(true);
				}
			}
			return(false);
		...
		public  boolean  doInsert(){
			...
			if(!tu.isEmptyValue(this.getMainxml())){
					...
					tu.applyObjectRule("Substruction",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_BEFORE,maindata);
					EventListener.registerPreInsertEvent("Substruction",maindata);
			}
			if(sql==null){
					if(ACONST.GENERATE_LOG)
			...
			if(ACONST.GENERATE_LOG)
			 ...
			 if (query.getTableResultset().getRowCount()>0){
				...
				if(ACONST.GENERATE_LOG)
					logger.info(usql);
				 tu.applyObjectRule("Substruction",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_AFTER,maindata);
				 tu.applyMTMRelation("Substruction","Substruction",getParentobjid());
				 EventListener.registerPostInsertEvent("Substruction",maindata);
				 return(true);
			}
			 return(false);
		}
