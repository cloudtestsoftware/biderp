	 ...
	 * @author S.K Jana Version 1.0 
	 ...
	 public class MachinaryImpl implements QueryEvent {
		 protected static Log logger = LogFactory.getLog(MachinaryImpl.class); 
		 ...
		 public void setAssetplandeleteid(String assetplandeleteid) {
		  ...
		  public  TemplateTable  doSelect(String[] column,String[] datatype,String parentfilter){
			...
			//do some custom pre query operation if any 
			 EventListener.registerPreQueryParent("Machinary",column,datatype);
			 ...
			 maindata=query.getTableResultset();
					...
					EventListener.registerPostQueryParent("Machinary",column,datatype);
			 return(maindata);
		 }
		 ...
		 public  TemplateTable  doSelectChild(String childname,String relfield,String pid,String[]column,String[]datatype,String childfilter){
			...
			EventListener.registerPreQueryChild("Machinary",childname,pid,relfield,column,datatype);
			...
			//Do some post query operation for child
					EventListener.registerPostQueryChild("Machinary",childname,pid,relfield,column,datatype);
			}
			return(data);
		}
		public  boolean   doDelete(String[] childtabs){
			...
			String sql=query.removeSql("Machinary",this.getParentobjid(),childtabs,groupuser);
			if(!tu.isEmptyValue(sql)){
				tu.applyObjectRuleForDelete("Machinary",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_BEFORE,this.getParentobjid());
				EventListener.registerPreDeleteEvent("Machinary",this.getParentobjid());
				query.setQuery(sql);
				if(ACONST.GENERATE_LOG)
					logger.info(query.getQuery());
				if (query.getTableResultset().getRowCount()>0){
					// Do some post delete operation
					tu.applyObjectRuleForDelete("Machinary",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_AFTER,this.getParentobjid());
					EventListener.registerPostDeleteEvent("Machinary",this.getParentobjid());
					return(true);
				}
			}
			return(false);
		...
		public  boolean  doInsert(){
			...
			if(!tu.isEmptyValue(this.getMainxml())){
					...
					tu.applyObjectRule("Machinary",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_BEFORE,maindata);
					EventListener.registerPreInsertEvent("Machinary",maindata);
			}
			if(sql==null){
					if(ACONST.GENERATE_LOG)
				...
				setParentobjid(query.getObjId()); 
				 if(!tu.isEmptyValue(this.getAssetplanxml()) ){
					 sql+=query.makeBulkSQL(false,getAssetplanxml(),"AssetPlan2Machinary",this.getUsername(),this.getGroupuser());
				 ...
				 usql+="\n\t\t"+tu.copyParentBQN2Child(this.getObject(),"AssetPlan","AssetPlan2Machinary",getParentobjid());
				}
			 }
			 sql+="\t\t end;";
			 query.setQuery(sql);
			if(ACONST.GENERATE_LOG)
			 ...
			 if (query.getTableResultset().getRowCount()>0){
				...
				if(ACONST.GENERATE_LOG)
					logger.info(usql);
				 tu.applyObjectRule("Machinary",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_AFTER,maindata);
				 tu.applyMTMRelation("Machinary","Machinary",getParentobjid());
				 EventListener.registerPostInsertEvent("Machinary",maindata);
				 tu.applyObjectRule("Assetplan",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_AFTER,assetplandata);
				 tu.applyMTMRelation("assetplan","Machinary",getParentobjid());
