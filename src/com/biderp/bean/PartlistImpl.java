	 ...
	 * @author S.K Jana Version 1.0 
	 ...
	 public class PartlistImpl implements QueryEvent {
		 ...
		 protected String partpricexml,partpricedeleteid;

		 protected TemplateTable partpricedata=new TemplateTable();
		 ...
		 public String getPartpricexml() {
			 return partpricexml;
		 }
		 public void setPartpricexml(String partpricexml) {
			 this.partpricexml=partpricexml;
		 }
		 public String getPartpricedeleteid() {
			 return partpricedeleteid;
		 }
		 public void setPartpricedeleteid(String partpricedeleteid) {
			 this.partpricedeleteid=partpricedeleteid;
		 }
		...
		public  boolean  doInsert(){
			...
			if(!tu.isEmptyValue(this.getMainxml())){
					sql=query.makeBulkSQL(true,this.getMainxml(),"PartList2BizDomain",this.getUsername(),this.getGroupuser());
			...
			if(sql==null){
				...
				setParentobjid(query.getObjId()); 
				 if(!tu.isEmptyValue(this.getPartpricexml()) ){
					 sql+=query.makeBulkSQL(false,getPartpricexml(),"PartPrice2PartList",this.getUsername(),this.getGroupuser());
					 partpricedata=query.getTableData();
					 tu.applyObjectRule("Partprice",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_BEFORE,partpricedata);
					 EventListener.registerPreInsertEvent("PartPrice",partpricedata);
				 usql+="\n\t\t"+tu.copyParentBQN2Child(this.getObject(),"PartPrice","PartPrice2PartList",getParentobjid());
				}
			 }
			 sql+="\t\t end;";
			 query.setQuery(sql);
			if(ACONST.GENERATE_LOG)
			 ...
			 if (query.getTableResultset().getRowCount()>0){
				...
				if(ACONST.GENERATE_LOG)
				 ...
				 tu.applyObjectRule("Partprice",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_AFTER,partpricedata);
				 tu.applyMTMRelation("partprice","Partlist",getParentobjid());
				 EventListener.registerPostInsertEvent("PartPrice",partpricedata);
				 return(true);
			}
			 return(false);
		}
