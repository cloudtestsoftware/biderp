	 ...
	 * @author S.K Jana Version 1.0 
	 ...
	 public class JoblistImpl implements QueryEvent {
		 protected static Log logger = LogFactory.getLog(JoblistImpl.class); 
		 ...
		 public void setTaskresourcedeleteid(String taskresourcedeleteid) {
		  ...
		  public  TemplateTable  doSelect(String[] column,String[] datatype,String parentfilter){
			...
			//do some custom pre query operation if any 
			 EventListener.registerPreQueryParent("Joblist",column,datatype);
			 ...
			 maindata=query.getTableResultset();
					...
					EventListener.registerPostQueryParent("Joblist",column,datatype);
			 return(maindata);
		 }
		 ...
		 public  TemplateTable  doSelectChild(String childname,String relfield,String pid,String[]column,String[]datatype,String childfilter){
			...
			EventListener.registerPreQueryChild("Joblist",childname,pid,relfield,column,datatype);
			...
			//Do some post query operation for child
					EventListener.registerPostQueryChild("Joblist",childname,pid,relfield,column,datatype);
			}
			return(data);
		}
		public  boolean   doDelete(String[] childtabs){
			...
			String sql=query.removeSql("Joblist",this.getParentobjid(),childtabs,groupuser);
			if(!tu.isEmptyValue(sql)){
				tu.applyObjectRuleForDelete("Joblist",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_BEFORE,this.getParentobjid());
				EventListener.registerPreDeleteEvent("Joblist",this.getParentobjid());
				query.setQuery(sql);
				if(ACONST.GENERATE_LOG)
					logger.info(query.getQuery());
				if (query.getTableResultset().getRowCount()>0){
					// Do some post delete operation
					tu.applyObjectRuleForDelete("Joblist",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_AFTER,this.getParentobjid());
					EventListener.registerPostDeleteEvent("Joblist",this.getParentobjid());
					return(true);
				}
			}
			return(false);
		...
		public  boolean  doInsert(){
			...
			if(!tu.isEmptyValue(this.getMainxml())){
					sql=query.makeBulkSQL(true,this.getMainxml(),"JobList2JobMaster",this.getUsername(),this.getGroupuser());
					maindata=query.getTableData();
					tu.applyObjectRule("Joblist",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_BEFORE,maindata);
					EventListener.registerPreInsertEvent("Joblist",maindata);
			}
			if(sql==null){
					if(ACONST.GENERATE_LOG)
				...
				setParentobjid(query.getObjId()); 
				 if(!tu.isEmptyValue(this.getPartsxml()) ){
					 sql+=query.makeBulkSQL(false,getPartsxml(),"Parts2JobList",this.getUsername(),this.getGroupuser());
				 ...
				 usql+="\n\t\t"+tu.copyParent2Child(maindata,"Parts",autofieldPartslist,"Parts2JobList",getParentobjid());
				 usql+="\n\t\t"+tu.copyParentBQN2Child(this.getObject(),"Parts","Parts2JobList",getParentobjid());
				}
				 if(!tu.isEmptyValue(this.getTaskresourcexml()) ){
					 sql+=query.makeBulkSQL(false,getTaskresourcexml(),"TaskResource2JobList",this.getUsername(),this.getGroupuser());
				 ...
				 usql+="\n\t\t"+tu.copyParent2Child(maindata,"TaskResource",autofieldTaskResourcelist,"TaskResource2JobList",getParentobjid());
				 usql+="\n\t\t"+tu.copyParentBQN2Child(this.getObject(),"TaskResource","TaskResource2JobList",getParentobjid());
				}
			 }
			 sql+="\t\t end;";
			 query.setQuery(sql);
			if(ACONST.GENERATE_LOG)
			 ...
			 if (query.getTableResultset().getRowCount()>0){
				...
				if(ACONST.GENERATE_LOG)
					logger.info(usql);
				 tu.applyObjectRule("Joblist",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_AFTER,maindata);
				 tu.applyMTMRelation("Joblist","Joblist",getParentobjid());
				 EventListener.registerPostInsertEvent("Joblist",maindata);
				 tu.applyObjectRule("Parts",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_AFTER,partsdata);
				 tu.applyMTMRelation("parts","Joblist",getParentobjid());
				 ...
				 tu.applyMTMRelation("taskresource","Joblist",getParentobjid());
