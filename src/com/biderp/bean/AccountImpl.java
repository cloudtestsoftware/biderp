	 ...
	 * @author S.K Jana Version 1.0 
	 ...
	 public class AccountImpl implements QueryEvent {
		 ...
		 protected String transactionxml,transactiondeleteid;

		 protected TemplateTable transactiondata=new TemplateTable();

		 public String AccountFilter="balance.AccountBalance Amount@account,balance@account.objid=balance.balance2account(+)";
		 ...
		 public String getTransactiondeleteid() {
			 return transactiondeleteid;
		 }
		 public void setTransactiondeleteid(String transactiondeleteid) {
			 this.transactiondeleteid=transactiondeleteid;
		 }
		...
		public  boolean  doInsert(){
			...
			if(sql==null){
				...
				setParentobjid(query.getObjId()); 
				 if(!tu.isEmptyValue(this.getTransactionxml()) ){
					 sql+=query.makeBulkSQL(false,getTransactionxml(),"Transaction2Account",this.getUsername(),this.getGroupuser());
					 transactiondata=query.getTableData();
					 tu.applyObjectRule("Transaction",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_BEFORE,transactiondata);
					 EventListener.registerPreInsertEvent("Transaction",transactiondata);
				 usql+="\n\t\t"+tu.copyParentBQN2Child(this.getObject(),"Transaction","Transaction2Account",getParentobjid());
				}
			 }
			 sql+="\t\t end;";
			 query.setQuery(sql);
			if(ACONST.GENERATE_LOG)
			 ...
			 if (query.getTableResultset().getRowCount()>0){
				...
				if(ACONST.GENERATE_LOG)
				 ...
				 tu.applyObjectRule("Transaction",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_AFTER,transactiondata);
				 tu.applyMTMRelation("transaction","Account",getParentobjid());
				 EventListener.registerPostInsertEvent("Transaction",transactiondata);
				 return(true);
			}
			 return(false);
		}
