	 ...
	 * @author S.K Jana Version 1.0 
	 ...
	 public class WhattodoImpl implements QueryEvent {
		 protected static Log logger = LogFactory.getLog(WhattodoImpl.class); 
		 ...
		 public void setLognotedeleteid(String lognotedeleteid) {
		  ...
		  public  TemplateTable  doSelect(String[] column,String[] datatype,String parentfilter){
			...
			//do some custom pre query operation if any 
			 EventListener.registerPreQueryParent("Whattodo",column,datatype);
			 ...
			 maindata=query.getTableResultset();
					...
					EventListener.registerPostQueryParent("Whattodo",column,datatype);
			 return(maindata);
		 }
		 ...
		 public  TemplateTable  doSelectChild(String childname,String relfield,String pid,String[]column,String[]datatype,String childfilter){
			...
			EventListener.registerPreQueryChild("Whattodo",childname,pid,relfield,column,datatype);
			...
			//Do some post query operation for child
					EventListener.registerPostQueryChild("Whattodo",childname,pid,relfield,column,datatype);
			}
			return(data);
		}
		public  boolean   doDelete(String[] childtabs){
			...
			String sql=query.removeSql("Whattodo",this.getParentobjid(),childtabs,groupuser);
			if(!tu.isEmptyValue(sql)){
				tu.applyObjectRuleForDelete("Whattodo",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_BEFORE,this.getParentobjid());
				EventListener.registerPreDeleteEvent("Whattodo",this.getParentobjid());
				query.setQuery(sql);
				if(ACONST.GENERATE_LOG)
					logger.info(query.getQuery());
				if (query.getTableResultset().getRowCount()>0){
					// Do some post delete operation
					tu.applyObjectRuleForDelete("Whattodo",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_AFTER,this.getParentobjid());
					EventListener.registerPostDeleteEvent("Whattodo",this.getParentobjid());
					return(true);
				}
			}
			return(false);
		...
		public  boolean  doInsert(){
			...
			if(!tu.isEmptyValue(this.getMainxml())){
					...
					tu.applyObjectRule("Whattodo",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_BEFORE,maindata);
					EventListener.registerPreInsertEvent("Whattodo",maindata);
			}
			if(sql==null){
					if(ACONST.GENERATE_LOG)
				...
				setParentobjid(query.getObjId()); 
				 if(!tu.isEmptyValue(this.getLognotexml()) ){
					 sql+=query.makeBulkSQL(false,getLognotexml(),"LogNote2WhatToDo",this.getUsername(),this.getGroupuser());
				 ...
				 usql+="\n\t\t"+tu.copyParentBQN2Child(this.getObject(),"LogNote","LogNote2WhatToDo",getParentobjid());
				}
			 }
			 sql+="\t\t end;";
			 query.setQuery(sql);
			if(ACONST.GENERATE_LOG)
			 ...
			 if (query.getTableResultset().getRowCount()>0){
				...
				if(ACONST.GENERATE_LOG)
					logger.info(usql);
				 tu.applyConsoleObject("Whattodo",maindata,this.getUsername(),groupuser,true);
				 tu.applyObjectRule("Whattodo",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_AFTER,maindata);
				 tu.applyMTMRelation("Whattodo","Whattodo",getParentobjid());
				 EventListener.registerPostInsertEvent("Whattodo",maindata);
				 tu.applyObjectRule("Lognote",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_AFTER,lognotedata);
				 tu.applyMTMRelation("lognote","Whattodo",getParentobjid());
