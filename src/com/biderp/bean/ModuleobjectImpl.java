	 ...
	 * @author S.K Jana Version 1.0 
	 ...
	 public class ModuleobjectImpl implements QueryEvent {
		 protected static Log logger = LogFactory.getLog(ModuleobjectImpl.class); 
		 ...
		 public void setObjectprivilegedeleteid(String objectprivilegedeleteid) {
		  ...
		  public  TemplateTable  doSelect(String[] column,String[] datatype,String parentfilter){
			...
			//do some custom pre query operation if any 
			 EventListener.registerPreQueryParent("Moduleobject",column,datatype);
			 ...
			 maindata=query.getTableResultset();
					...
					EventListener.registerPostQueryParent("Moduleobject",column,datatype);
			 return(maindata);
		 }
		 ...
		 public  TemplateTable  doSelectChild(String childname,String relfield,String pid,String[]column,String[]datatype,String childfilter){
			...
			EventListener.registerPreQueryChild("Moduleobject",childname,pid,relfield,column,datatype);
			...
			//Do some post query operation for child
					EventListener.registerPostQueryChild("Moduleobject",childname,pid,relfield,column,datatype);
			}
			return(data);
		}
		public  boolean   doDelete(String[] childtabs){
			...
			String sql=query.removeSql("Moduleobject",this.getParentobjid(),childtabs,groupuser);
			if(!tu.isEmptyValue(sql)){
				tu.applyObjectRuleForDelete("Moduleobject",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_BEFORE,this.getParentobjid());
				EventListener.registerPreDeleteEvent("Moduleobject",this.getParentobjid());
				query.setQuery(sql);
				if(ACONST.GENERATE_LOG)
					logger.info(query.getQuery());
				if (query.getTableResultset().getRowCount()>0){
					// Do some post delete operation
					tu.applyObjectRuleForDelete("Moduleobject",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_AFTER,this.getParentobjid());
					EventListener.registerPostDeleteEvent("Moduleobject",this.getParentobjid());
					return(true);
				}
			}
			return(false);
		...
		public  boolean  doInsert(){
			...
			if(!tu.isEmptyValue(this.getMainxml())){
					...
					tu.applyObjectRule("Moduleobject",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_BEFORE,maindata);
					EventListener.registerPreInsertEvent("Moduleobject",maindata);
			}
			if(sql==null){
					if(ACONST.GENERATE_LOG)
				...
				setParentobjid(query.getObjId()); 
				 if(!tu.isEmptyValue(this.getObjectprivilegexml()) ){
					 sql+=query.makeBulkSQL(false,getObjectprivilegexml(),"ObjectPrivilege2ModuleObject",this.getUsername(),this.getGroupuser());
				 ...
				 usql+="\n\t\t"+tu.copyParentBQN2Child(this.getObject(),"ObjectPrivilege","ObjectPrivilege2ModuleObject",getParentobjid());
				}
			 }
			 sql+="\t\t end;";
			 query.setQuery(sql);
			if(ACONST.GENERATE_LOG)
			 ...
			 if (query.getTableResultset().getRowCount()>0){
				...
				if(ACONST.GENERATE_LOG)
					logger.info(usql);
				 tu.applyObjectRule("Moduleobject",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_AFTER,maindata);
				 tu.applyMTMRelation("Moduleobject","Moduleobject",getParentobjid());
				 EventListener.registerPostInsertEvent("Moduleobject",maindata);
				 tu.applyObjectRule("Objectprivilege",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_AFTER,objectprivilegedata);
				 tu.applyMTMRelation("objectprivilege","Moduleobject",getParentobjid());
