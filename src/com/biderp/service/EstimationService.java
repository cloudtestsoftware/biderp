		...
		import com.biderp.dao.EstimationDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class EstimationService {
			...
			// Get all Boq rows against object ID for Estimation
			@GET
			@Path("/{id}/boq")
			@Produces({"application/xml"})
			public Rows getBoqRows(@Context UriInfo uriInfo,@Context  HttpHeaders header) {
				Rows rows = null;
				try {
					rows=new EstimationDao(uriInfo,header).getBoqRows();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
