		...
		import com.biderp.dao.CompanyDao;
		/*
		*  URL Parameters:
		*  
		...
		*/
		...
		public class CompanyService {
			...
			// Get all Currency rows against object ID for Company
			@GET
			@Path("/{id}/currency")
			@Produces({"application/xml"})
			public Rows getCurrencyRows(@Context UriInfo uriInfo,@Context  HttpHeaders header) {
				Rows rows = null;
				try {
					rows=new CompanyDao(uriInfo,header).getCurrencyRows();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
			...
			// Get all User rows against object ID for Company
			@GET
			@Path("/{id}/user")
			@Produces({"application/xml"})
			public Rows getUserRows(@Context UriInfo uriInfo,@Context  HttpHeaders header) {
				Rows rows = null;
				try {
					rows=new CompanyDao(uriInfo,header).getUserRows();
				} catch (AuthenticationException e) {
					 rows=new TemplateUtility().getFailedMessage(e.getMessage());
					 e.printStackTrace();
				} catch (Exception ex) {
