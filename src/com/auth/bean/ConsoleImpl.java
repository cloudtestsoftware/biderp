	 ...
	 * @author S.K Jana Version 1.0 
	 ...
	 public class ConsoleImpl {
		 protected static Log logger = LogFactory.getLog(ConsoleImpl.class); 
		 ...
		 protected String searchdata,object,parentobjid,filters; 
		 ...
		 public String getParentobjid(){
		  ...
		  public  TemplateTable  DogetPostSelect(String[] column,String[] datatype,String parentfilter){
			...
			//do some custom pre query operation if any 
			 EventListener.registerPreQueryParent("Console",column,datatype);
			 ...
			 maindata=query.getTableResultset();
					...
					EventListener.registerPostQueryParent("Console",column,datatype);
			 return(maindata);
		 }
		 ...
		 public  TemplateTable  DogetPostSelectChild(String childname,String relfield,String pid,String[]column,String[]datatype,String childfilter){
			...
			EventListener.registerPreQueryChild("Console",childname,pid,relfield,column,datatype);
			...
			//Do some post query operation for child
					EventListener.registerPostQueryChild("Console",childname,pid,relfield,column,datatype);
			}
			return(data);
		}
		public  boolean   DogetPostDelete(){
			TemplateQuery query =new TemplateQuery();
			// Do some pre delete operation 
			tu.applyObjectRuleForDelete("Console",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_BEFORE,this.getParentobjid());
			EventListener.registerPreDeleteEvent("Console",this.getParentobjid());
			String sql=query.makeBulkDelete("Console",this.getParentobjid(),ACONST.BULK_DELETE_MODE_REMOVE_RELATION);
			query.setQuery(sql);
			if(ACONST.GENERATE_LOG)
					logger.info(query.getQuery());
			if (query.getTableResultset().getRowCount()>0){
				// Do some post delete operation
				tu.applyObjectRuleForDelete("Console",ACONST.EVENT_REASON_DELETE, ACONST.EVENT_STATE_AFTER,this.getParentobjid());
				EventListener.registerPostDeleteEvent("Console",this.getParentobjid());
				return(true);
			}
			return(false);
		}
		...
		if(!tu.isEmptyValue(this.getMainxml())){
				...
				tu.applyObjectRule("Console",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_BEFORE,maindata);
				EventListener.registerPreInsertEvent("Console",maindata);
		}
		if(sql==null){
				if(ACONST.GENERATE_LOG)
			...
			if(ACONST.GENERATE_LOG)
			 ...
			 if (query.getTableResultset().getRowCount()>0){
				 ...
				 tu.applyConsoleObject("Console",maindata,this.getUsername(),groupuser,true);
				 tu.applyObjectRule("Console",ACONST.EVENT_REASON_INSERT, ACONST.EVENT_STATE_AFTER,maindata);
				 tu.applyMTMRelation("Console","Console",getParentobjid());
				 EventListener.registerPostInsertEvent("Console",maindata);
				 return(true);
			}
			 return(false);
		}
